...
def test_match_custom_names_with_location_codes_extra_whitespace_complicated():
    s = Flow({"name": "Water (ersatz), net cons., irrigation,  \t RER w/o DE+NL+NO", "context": "air", "unit": "kg"})
    t = Flow({"name": "water, unspecified natural origin", "context": "air", "unit": "kg"})

    actual = match_custom_names_with_location_codes(s, t)
    expected = {"comment": "Custom names with location code", "location": "RER w/o DE+NL+NO", "irrigation": True}
    assert actual == expected
...
def test_match_custom_names_with_location_codes_conversion():
    ...
    t = Flow({"name": "Water, well, in ground", "context": "air", "unit": "cubic_meter"})

    actual = match_custom_names_with_location_codes(s, t)
    expected = {"comment": "Custom names with location code", "location": "HU", "conversion_factor": 0.001}
    assert actual == expected
    ...
    actual = match_custom_names_with_location_codes(s, t)
    expected = {"comment": "Custom names with location code", "location": "HU", "conversion_factor": 1000.}
    assert actual == expected
